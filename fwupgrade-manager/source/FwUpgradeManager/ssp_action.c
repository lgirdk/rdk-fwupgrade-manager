...
ANSC_STATUS ssp_create()
{
    ...
    g_pComponentCommonFwUpgradeMgr = (PCOMPONENT_COMMON_PLATFORM_MANAGER) AnscAllocateMemory(sizeof(COMPONENT_COMMON_PLATFORM_MANAGER));

    if(!g_pComponentCommonFwUpgradeMgr)
    {
        return ANSC_STATUS_RESOURCES;
    }

    ComponentCommonDmInit(g_pComponentCommonFwUpgradeMgr);

    g_pComponentCommonFwUpgradeMgr->Name    = AnscCloneString(RDK_COMPONENT_NAME_PLATFORM_MANAGER);
    g_pComponentCommonFwUpgradeMgr->Version = 1;
    g_pComponentCommonFwUpgradeMgr->Author  = AnscCloneString("Your Name");
    ...
    if (!pDslhLcbIf)
    {
        ...
        else
        {
            ...
            pDslhLcbIf->InitLibrary              = FirmwareUpgradeDmlInit;
        }
    }
    ...
    )
{
    ...
    g_pComponentCommonFwUpgradeMgr->Health = RDK_COMMON_COMPONENT_HEALTH_Yellow;
    ...
    if ( returnStatus == ANSC_STATUS_SUCCESS || returnStatus == CCSP_SUCCESS)
    {
        ...
        g_pComponentCommonFwUpgradeMgr->Health = RDK_COMMON_COMPONENT_HEALTH_Green;
    }
    ...
    )
{
    ...
    if(  g_pComponentCommonFwUpgradeMgr == NULL)
    {
        return ANSC_STATUS_SUCCESS;
    }
    ...
    {
        sprintf(CrName, "%s%s", g_Subsystem, CCSP_DBUS_INTERFACE_CR);
        sprintf(CpName, "%s%s", g_Subsystem, RDK_COMPONENT_NAME_PLATFORM_MANAGER);
    }
    else
    {
        sprintf(CrName, "%s", CCSP_DBUS_INTERFACE_CR);
        sprintf(CpName, "%s", RDK_COMPONENT_NAME_PLATFORM_MANAGER);
    }
    ...
    if (  g_pComponentCommonFwUpgradeMgr ) AnscFreeMemory( g_pComponentCommonFwUpgradeMgr);

     g_pComponentCommonFwUpgradeMgr = NULL;
    pSsdCcdIf                = NULL;
...
{
    return  g_pComponentCommonFwUpgradeMgr->Name;
}
...
{
    return  g_pComponentCommonFwUpgradeMgr->Version;
}
...
{
    return  g_pComponentCommonFwUpgradeMgr->Author;
}
...
{
    return  g_pComponentCommonFwUpgradeMgr->Health;
}
...
{
    return  g_pComponentCommonFwUpgradeMgr->State;
}
...
{
    return  g_pComponentCommonFwUpgradeMgr->LogEnable;
}
...
{
    if(g_pComponentCommonFwUpgradeMgr->LogEnable == bEnabled) 
    {
        return ANSC_STATUS_SUCCESS;
    }

    g_pComponentCommonFwUpgradeMgr->LogEnable = bEnabled;
        
    if(bEnabled) 
    {
        g_iTraceLevel = (INT) g_pComponentCommonFwUpgradeMgr->LogLevel;
    }
    else 
    {
...
{
    return  g_pComponentCommonFwUpgradeMgr->LogLevel;
}
...
{
    if(g_pComponentCommonFwUpgradeMgr->LogLevel == LogLevel) 
    {
        return ANSC_STATUS_SUCCESS;
    }

    g_pComponentCommonFwUpgradeMgr->LogLevel = LogLevel;

    if(g_pComponentCommonFwUpgradeMgr->LogEnable) 
    {
        g_iTraceLevel = (INT) g_pComponentCommonFwUpgradeMgr->LogLevel;
    }
...
{
    return  g_pComponentCommonFwUpgradeMgr->MemMinUsage;
}
    ...
    )
{
    ...
    size = AnscGetComponentMemorySize(RDK_COMPONENT_NAME_PLATFORM_MANAGER);
    if (size == -1 )
        size = 0;
    ...
    )
{
    ...
    /* AnscSetTraceLevel((INT) g_pComponentCommonFwUpgradeMgr->LogLevel); */
    return returnStatus;
}
